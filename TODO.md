...
## 완료
- [x] 태블릿 Safari에서 HTML 원문 뷰어 스크롤 안 되는 문제 수정 — iframe `contentWindow`에 직접 붙인 터치 리스너(`iframeTouch`)에 의존하던 구조가 원인. 별도 브라우징 컨텍스트(iframe) 안에서 발생한 포인터 이벤트를 iOS Safari가 부모 리스너로 확실히 전달하지 않음(w3c/pointerevents#325, WebKit 이슈 149264). iframe에 `pointer-events: none`을 걸어 모든 터치/휠을 최상위 스크롤 박스(`.html-viewer-scroll`)로 떨어뜨리고 PDF와 동일한 구조(같은 문서 안 div에 제스처 처리)로 통일. 트레이드오프로 스냅샷 콘텐츠 위 텍스트 드래그 선택/링크 클릭은 막힘(사용 빈도 낮아 수용하기로 확인받음). 빌드 통과. 실기기(iPad Safari) 최종 확인 필요
