...
const steps=[
{eyebrow:'계정 만들기',title:'Zotero 홈페이지에서 가입을 시작해요',description:'처음이라면 Zotero 계정을 먼저 만들어야 합니다. 화면의 회원가입 메뉴를 찾아 가입 페이지로 이동하세요.',tip:'이미 Zotero 계정이 있다면 이 단계는 건너뛰어도 괜찮아요.',image:'/images/zotero-setup/1.png',alt:'Zotero 회원가입 시작 화면'},
{eyebrow:'정보 입력',title:'사용할 계정 정보를 입력하세요',description:'사용자 이름과 이메일, 비밀번호를 차례로 입력합니다. 앞으로 Folio에 연결할 때도 같은 Zotero 계정을 사용하게 됩니다.',tip:'자주 확인하는 이메일을 사용하면 계정 인증과 복구가 쉬워요.',image:'/images/zotero-setup/2.png',alt:'Zotero 계정 정보 입력 화면'},
{eyebrow:'가입 완료',title:'입력한 내용을 확인하고 가입을 마쳐요',description:'약관과 필수 항목을 확인한 뒤 가입 버튼을 누르세요. 오류 안내가 보이면 표시된 입력란만 다시 확인하면 됩니다.',tip:'비밀번호는 다른 서비스와 겹치지 않게 설정해 주세요.',image:'/images/zotero-setup/3.png',alt:'Zotero 회원가입 확인 화면'},
{eyebrow:'이메일 확인',title:'받은 편지함에서 인증을 완료하세요',description:'Zotero가 보낸 확인 메일을 열고 인증 링크를 눌러 주세요. 메일이 바로 오지 않으면 스팸함도 확인해 보세요.',tip:'메일 도착에는 몇 분 정도 걸릴 수 있어요.',image:'/images/zotero-setup/4.png',alt:'Zotero 이메일 인증 안내 화면'},
{eyebrow:'Folio 연결',title:'Folio에서 Zotero 연결을 선택하세요',description:'계정 준비가 끝났다면 Folio로 돌아와 Zotero 연결 버튼을 누릅니다. Zotero의 안전한 인증 화면으로 이동해요.',tip:'Folio는 Zotero 비밀번호를 직접 저장하지 않습니다.',image:'/images/zotero-setup/5.png',alt:'Folio와 Zotero 계정 연결 화면'},
{eyebrow:'접근 허용',title:'권한을 확인하고 연결을 허용하세요',description:'연결할 계정과 접근 범위를 확인한 뒤 허용해 주세요. 승인이 끝나면 Folio의 나머지 초기 설정을 이어갈 수 있습니다.',tip:'연결 후에도 Zotero 계정 설정에서 언제든 권한을 해제할 수 있어요.',image:'/images/zotero-setup/6.png',alt:'Zotero 접근 권한 승인 화면'},
{eyebrow:'브라우저 확장 프로그램',title:'Zotero Connector를 추가하면 더 편리해요',description:'Chrome에 Zotero Connector를 설치하면 웹에서 찾은 논문과 자료를 클릭 한 번으로 Zotero 라이브러리에 저장할 수 있습니다.',tip:'필수 설치는 아니지만 웹에서 논문을 자주 찾는다면 함께 사용하는 것을 권장해요.',image:'/images/zotero-setup/7.png',alt:'Chrome에 Zotero Connector 확장 프로그램을 추가하는 화면'},
{eyebrow:'첫 동기화',title:'내 Zotero 라이브러리를 불러오세요',description:'첫 동기화를 시작하면 Zotero에 보관한 논문, 첨부 파일과 컬렉션 정보를 Folio가 차례로 불러옵니다.',tip:'자료가 많을수록 첫 동기화에는 시간이 조금 더 걸릴 수 있어요.',image:'/images/zotero-setup/8.png',alt:'Zotero 라이브러리 첫 동기화 화면'},
{eyebrow:'라이브러리 확인',title:'불러온 논문과 컬렉션을 확인해요',description:'동기화가 끝나면 라이브러리에서 자료 목록을 살펴보세요. Zotero에서 정리한 컬렉션 구조도 함께 확인할 수 있습니다.',tip:'일부 자료가 없다면 Zotero 동기화를 마친 뒤 Folio에서 다시 동기화해 보세요.',image:'/images/zotero-setup/9.png',alt:'Folio에 불러온 Zotero 라이브러리 화면'},
{eyebrow:'설정 완료',title:'첫 논문을 열면 모든 준비가 끝나요',description:'읽고 싶은 논문을 선택해 PDF와 메모 화면을 열어 보세요. 이제 하이라이트와 기록을 한곳에서 이어갈 수 있습니다.',tip:'원본 논문과 서지 정보는 Zotero에 그대로 안전하게 남아 있어요.',image:'/images/zotero-setup/10.png',alt:'Folio에서 논문을 여는 화면'}];
